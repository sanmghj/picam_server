...
from config import (
    DEFAULT_VIDEO_DIR, DEFAULT_VIDEO_FORMAT,
    DEFAULT_WIDTH, DEFAULT_HEIGHT, DEFAULT_FPS,
    GC_INTERVAL_RECORDING, GC_INTERVAL_STREAMING,
    MEM_LOG_INTERVAL_RECORDING, MEM_LOG_INTERVAL_STREAMING,
    BUFFER_LOG_INTERVAL,
...
class CameraManager:
    ...
    def __init__(self, video_dir=DEFAULT_VIDEO_DIR):
        ...
        self.camera = None
        self.camera_mode = None  # 'recording', 'streaming', None
    ...
    def start_recording_thread(self):
        ...
        def record_video_thread():
            try:
                ...
                loop_count = 0
                gc_interval = GC_INTERVAL_RECORDING
                mem_log_interval = MEM_LOG_INTERVAL_RECORDING
    ...
    def generate_stream(self):
        ...
        try:
            ...
            if self.camera is None:
                try:
                    logger.info("Initializing camera for streaming")
                ...
                logger.info("Reusing existing camera for streaming")
                self.streaming = True
                self.camera_mode = 'streaming'

            # 프레임 생성 루프 (8GB RAM 최적화)
            frame_count = 0
            gc_interval = GC_INTERVAL_STREAMING
            total_buffer_size = 0  # 누적 버퍼 사용량
            buffer_log_interval = BUFFER_LOG_INTERVAL

            while self.streaming and self.camera is not None:
                buffer = None  # 명시적 초기화
                try:
                    # 카메라 객체가 None이 아닌지 다시 확인
                    if self.camera is None:
                        logger.warning("Camera became None during streaming")
                        break
                    ...
                    # 30분마다 메모리 상태 로깅
                    if frame_count % MEM_LOG_INTERVAL_STREAMING == 0:
                        mem = psutil.virtual_memory()
    ...
    def stop_stream(self):
        ...
        if self.camera and self.camera_mode == 'streaming':
            try:
