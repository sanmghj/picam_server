...
class APIHandlers:
    ...
    @staticmethod
    def start_recording(camera_manager):
        """녹화 시작 핸들러"""
    ...
    @staticmethod
    def download_video(camera_manager):
        """MP4 변환된 파일 다운로드"""
        logger.info(f"[/download] Request received from {request.remote_addr}")

        if camera_manager.is_converting():
        ...
        video_path = camera_manager.video_path
        ...
        if os.path.exists(video_path):
            ...
            try:
                response = send_file(
                    video_path,
                    as_attachment=True,
                    download_name=f"camera_video.{camera_manager.video_format}",
                    max_age=0
                )
    ...
    @staticmethod
    def download_raw_video(camera_manager):
        """H264 원본 파일 즉시 다운로드"""
        ...
        temp_path = camera_manager.temp_path
    ...
    def get_status(camera_manager):
        ...
        if camera_manager.is_converting():
        ...
        elif camera_manager.is_recording():
            elapsed = camera_manager.get_recording_duration()
            ...
            return jsonify({
                ...
                "start_time": camera_manager.recording_start_time
            }), 200
        else:
    ...
    def test_camera(camera_manager):
        ...
        try:
            output_path = camera_manager.test_camera()
    ...
    def stop_stream(camera_manager):
        ...
        try:
            camera_manager.force_stop_stream()
